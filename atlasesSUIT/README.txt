Probabilistic Cerebellar Atlas in SUIT space for use with MRIcron. 
 http://www.mccauslandcenter.sc.edu/mricro/mricron/

Use this probabilistic Atlas only with imaging data aligned to SUIT space (Diedrichsen, 2006). 
http://www.icn.ucl.ac.uk/motorcontrol/imaging/suit.htm

While SUIT is spatially unbiased in relationship to MNI space (thus is in MNI coordinates!), 
the alignment is better with SUIT than with any other tested whole brain alignment method. 
For data aligned with whole brain normalization methods, use the Cerebellar Atlas in MNI space after 
affine or non-linear alignment. 

Cerebellum-SUIT.nii.gz: 	Maximum probability map signifying the most probable compartment (1-28) 
Cerebellum-SUIT.txt: 		Assignment of compartment numbers to lobules 
Cerebellum-SUIT-maxprob.nii: 	Probability of the compartment with the highest probability 
Cerebellum-SUIT-prob.nii: 	28 full probability maps for each of the compartments 

PLEASE CITE: 
  Diedrichsen, J., Balsters, J. H., Flavell, J., Cussans, E.,
   & Ramnani, N. (2009). A probabilistic atlas of the human
   cerebellum. Neuroimage.
 Diedrichsen, J., Maderwald, S., Kuper, M., Thurling, M.,
   Rabe, K., Gizewski, E. R., et al. (2011). Imaging the deep
   cerebellar nuclei: A probabilistic atlas and normalization
   procedure. Neuroimage

For more information: 
http://www.icn.ucl.ac.uk/motorcontrol/imaging/propatlas.htm

The Buckner* files contain an atlas based on the connectivity between cerebellum and neocortex in 
SUIT space, as described in: 

Buckner RL, Krienen FM, Castellanos A, Diaz JC, Yeo BT (2011) The organization of the human cerebellum 
estimated by intrinsic functional connectivity. J Neurophysiol 106:2322-2345.

see also: 
http://www.icn.ucl.ac.uk/motorcontrol/imaging/suit_connect.htm

